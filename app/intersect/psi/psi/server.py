...
class Server(object):
    def __init__(self, pair: Pair, words: List[bytes]):
        self.s = 5              # stash size
        self.words = words      # 待求交数据

        self.oprf_server = OprfServer(pair, 128)
        self.pair = pair
    ...
    def intersect(self):
        n = self.oprf_server.max_count - self.s                     # n为cuckoo中桶的个数
        for i in range(3):
            words = random.sample(self.words, len(self.words))      # 打乱数据
            for word in words:
